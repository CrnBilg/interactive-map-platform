...
export default function ProfilePage() {
  ...
  if (!profile) return <div className="flex items-center justify-center h-64 text-stone-500">Yükleniyor...</div>
        ...
        <div className="md:col-span-2 space-y-5">
          ...
          {editing && (
            <form onSubmit={handleUpdate} className="card p-5 space-y-4">
              <h3 className="font-semibold text-stone-200">Profili Düzenle</h3>
              <div>
                <label className="block text-stone-400 text-sm mb-1.5">Kullanıcı Adı</label>
                <input className="input" value={form.username} onChange={e => setForm(f => ({ ...f, username: e.target.value }))} />
              </div>
              <div>
                <label className="block text-stone-400 text-sm mb-1.5">Bio</label>
                <textarea className="input resize-none" rows={3} placeholder="Kendini tanıt..." value={form.bio} onChange={e => setForm(f => ({ ...f, bio: e.target.value }))} />
              </div>
              <div className="flex gap-2">
                <button type="submit" disabled={loading} className="btn-primary text-sm disabled:opacity-50">
                  {loading ? 'Kaydediliyor...' : 'Kaydet'}
                </button>
                <button type="button" onClick={() => setEditing(false)} className="btn-secondary text-sm">İptal</button>
              </div>
            </form>
          )}
          ...
          <div className="card p-5">
            <h3 className="font-semibold text-stone-200 mb-4 flex items-center gap-2">
              <Bookmark size={16} className="text-amber-400" /> Kaydedilen Mekanlar
            </h3>
            {profile.savedPlaces?.length === 0 ? (
              <div className="text-stone-600 text-sm text-center py-4">
                Henüz kaydedilen mekan yok. <Link to="/map" className="text-amber-400 hover:underline">Keşfet!</Link>
              </div>
            ) : (
              <div className="space-y-2">
                {profile.savedPlaces?.map(place => (
                  <Link key={place._id} to={`/place/${place._id}`} className="flex items-center gap-3 p-3 bg-stone-800 rounded-xl hover:bg-stone-700/50 transition-colors">
                    ...
                    <div>
                      <div className="font-medium text-stone-200 text-sm">{place.name}</div>
                      <div className="text-stone-500 text-xs">{place.city}</div>
                    </div>
                  </Link>
                ))}
              </div>
